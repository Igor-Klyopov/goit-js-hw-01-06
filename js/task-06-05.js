...
//вариант 1 (не возвращает значение, если оно равно 0)
// const getAllPropValues = propName => {
//   return products.map(prop => prop[propName]).filter(prop => prop);
// };

//вариант 2 (возвращает значение, даже если оно равно 0)
// const getAllPropValues = propName => {
//   return products
//     .filter(prop => prop[propName] !== undefined)
//     .map(prop => prop[propName]);
// };

//вариант 3 (в один проход, лучше по производительности)
const getAllPropValues = propName => {
  return products.reduce((propNameArr, prop) => {
    if (prop[propName] !== undefined) {
      propNameArr.push(prop[propName]);
    }
    return propNameArr;
  }, []);
};
